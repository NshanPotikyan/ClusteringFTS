...
def extract_symbols(df, extension):
    """
    extension = {
    'TL' - Tallinn
    'RG' - Riga
    ...
    """
    df['Name'], df['Symbol'] = df['Name'].str.replace(')', '').str.split('(').str
    df['Name'] = df.Name.str.strip()
    df['Symbol'] = df.Symbol + '.' + extension
    return df


def get_stock_info(dfs, extensions):
    out_df = extract_symbols(dfs[0], extensions[0])
    for i in range(1, len(dfs)):
        out_df = pd.concat([out_df, extract_symbols(dfs[i], extensions[i])])
    return out_df


def get_closed_prices(df):
    out_df = df.copy()
    #     out_df.columns = ['_'.join(col).strip() for col in out_df.columns.values]
    filtered = [col for col in out_df.columns if col.split('_')[-1] == 'Close']
    #     filtered.insert(0, 'Date')
    out_df = out_df.loc[:, filtered]
    out_df = out_df.loc[:, out_df.count() > 200]
    print('Shape: ', out_df.shape)
    return out_df


def get_info(stock_id, feature, df, stock_info):
    """

    :param stock_id:
    :param feature: 'Name', 'Sector' or other info related to stock companies
    :param df:
    :param stock_info:
    :return:
    """
    selected = stock_info.Symbol == df.columns[stock_id].split('_')[0]
    return stock_info.loc[selected, feature].values[0]


def get_sectors(df, stock_info):
    """
    Get the sectors of the stocks from the given dataframe
    :param df:
    dataframe containing stock names as columns
    :param stock_info:
    dataframe containing the stock sector info (Sector)
    and stock name (Name) as columns
    :return:
    pandas series
    """
    sectors = []
    for col_name in df.columns:
        mask = stock_info.Symbol == col_name.split('_')[0]
        sectors.extend(stock_info.Sector[mask].values)
    return pd.Series(sectors)


def get_revenues(df, stock_info):
    """
    Get the sectors of the stocks from the given dataframe
    :param df:
    dataframe containing stock names as columns
    ...
    for col_name in df.columns:
        mask = stock_info.Symbol == col_name.split('_')[0]
        revs.extend(stock_info.Value_total[mask].values)
